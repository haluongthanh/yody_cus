...
const ProductPreview = ({ previewImages }) => {
  ...
  useEffect(() => {
    if (previewImages.length > 0) {
      setActivePreviewImage(previewImages[0]?.link);
    }
  }, [previewImages]);
  ...
  return (
    <ProductPreviewWrapper className="grid items-center">
        ...
        {previewImages?.map((previewImage) => {
          return (
            <div
              ...
              key={previewImage.id}
              onClick={() => handlePreviewImageChange(previewImage.link)}
            >
              <div className="preview-item">
                <img
                  src={`https://api.yody.lokid.xyz${previewImage.link}`}
                  alt=""
      ...
      <div className="preview-display">
        <img
          src={`https://api.yody.lokid.xyz${activePreviewImage}`}
          className="object-fit-cover"
          alt=""
        />
      </div>
